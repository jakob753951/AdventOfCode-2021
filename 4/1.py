...
def check_board(board: list[tuple[int, bool]]) -> bool:
	...
	rows_check = any([all(bool_board[i:i+5]) for i in range(5)])
...
for number in balls:
	...
	if any([check_board(board) for board in boards]):
		winning_board = [board for board in boards if check_board(board)][0]
...
print()
print(sum([cell[0] for cell in winning_board if not cell[1]])*winning_board)
